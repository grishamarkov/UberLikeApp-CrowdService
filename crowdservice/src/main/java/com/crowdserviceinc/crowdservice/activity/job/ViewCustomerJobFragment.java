...
import com.crowdserviceinc.crowdservice.activity.job.ViewJobsActivity.BackListener;
import com.crowdserviceinc.crowdservice.adapter.ViewCustomerJobListingAdapter;
...
import com.crowdserviceinc.crowdservice.widget.swipetodelete.SwipeMenu;
import com.crowdserviceinc.crowdservice.widget.swipetodelete.SwipeMenuCreator;
import com.crowdserviceinc.crowdservice.widget.swipetodelete.SwipeMenuItem;
import com.crowdserviceinc.crowdservice.widget.swipetodelete.SwipeMenuListView;
import com.crowdserviceinc.crowdservice.widget.swipetodelete.SwipeMenuListView.OnMenuItemClickListener;
...
public class ViewCustomerJobFragment extends Fragment implements
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		JSONObject activeJobsEvent = new JSONObject();

		String userId = Constants.uid;
		String userName = Constants.username;

		try {
			activeJobsEvent.put("User", userName);
			activeJobsEvent.put("UserId", userId);
			mixpanel.track("ActiveJobsEvent", activeJobsEvent);
	...
	public void onActivityCreated(Bundle savedInstanceState) {
		...
		is_Customer = Constants.is_customer;
		initUi();
	}
		...
		public void handleMessage(Message msg) {
			...
			if (msg.what == JOB_LIST && msg.arg1 == SUCCESS) {
				...
				if (model.getStatus().equalsIgnoreCase("success")) {
					JobDatalist = reorderJobList(new ArrayList<JobData>(Arrays.asList(model
							.getJobData())));
	...
	public void onItemClick(AdapterView<?> arg0, View view, int position,
		...
		if(is_Customer){
			...
			if (x == 0) {
				...
				bundle.putBoolean("jobAccept", false);
...
//				bundle.putSerializable("merch", job.getJob().getAmount());
				...
				if (bid.getLinkStatus().contains("A")
					...
					Log.d("From Customer", "Has bidder, but already active or completed");
...
//		alert.show();
	...
	private ArrayList<JobData> reorderJobList(ArrayList<JobData> list){
		...
		ArrayList<JobData> tempA = new ArrayList<JobData>();
		ArrayList<JobData> tempB = new ArrayList<JobData>();
		ArrayList<JobData> tempC = new ArrayList<JobData>();
		ArrayList<JobData> tempD = new ArrayList<JobData>();
		
		for(JobData data : list){
